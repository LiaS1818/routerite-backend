...
export class MailerService {
  ...
  async sendEmailVerification(to: string, token: string): Promise<void> {
    ...
    const templatePath = path.join(process.cwd(), 'src', 'mailer', 'templates', 'email-verification.hbs');

    try {
      console.log('Enviando email de verificación a:', to);
      console.log('Template dir:', path.join(process.cwd(), 'src', 'mailer', 'templates'));
  ...
  async sendPasswordResetEmail(to: string, token: string): Promise<void> {
    ...
    const templatePath = path.join(process.cwd(), 'src', 'mailer', 'templates', 'password-reset.hbs');

    try {
      console.log('Enviando email de restablecimiento de contraseña a:', to);
      console.log('Template dir:', path.join(process.cwd(), 'src', 'mailer', 'templates'));
      console.log('Template file exists:', fs.existsSync(templatePath));
      ...
      console.log('Email de restablecimiento enviado exitosamente:', result);
    } catch (error) {
